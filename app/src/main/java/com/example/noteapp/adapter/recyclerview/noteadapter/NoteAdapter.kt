...
class NoteAdapter : ListAdapter<DataItem, RecyclerView.ViewHolder>(DiffCallback){
    ...
    fun pinnedSorting(list: List<Note>, isGridLayout: Boolean) {
        adapterScope.launch {
            ...
            val finalList =
                if (isGridLayout) {
                    listOf(DataItem.Header("Others")) + listOf(DataItem.Header("")) + notPinned.map {
                        DataItem.NoteItem(
                            it
                        )
                    }
                } else {
                    listOf(DataItem.Header("Others")) + notPinned.map { DataItem.NoteItem(it) }
                }

            withContext(Dispatchers.Main) {
                if (pinned.isEmpty()) {
                    submitList(notPinned.map { DataItem.NoteItem(it) })
                } else {
